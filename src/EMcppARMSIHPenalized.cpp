...
Xi FilterARMSIHPenalized (arma::colvec* py,
                arma::mat* py_lagged,
                arma::mat* pz_dependent,
                arma::mat* pz_independent,
                Theta* ptheta,
                arma::colvec sigma_null,
                double penalty_term)
{
  int n = py->n_rows;
...
Xi ExpectationStepARMSIHPenalized	(arma::colvec* py,
                          arma::mat* py_lagged,
                          arma::mat* pz_dependent,
                          arma::mat* pz_independent,
                          Theta* ptheta,
                          ...
                          double penalty_term)
{
  Xi filter = FilterARMSIHPenalized(py, py_lagged, pz_dependent, pz_independent,
                            ptheta,
                            sigma_null,
                            penalty_term);
...
Theta MaximizationStepARMSIHPenalized (arma::colvec* py,
                            arma::mat* py_lagged,
                            arma::mat* pz_dependent,
                            arma::mat* pz_independent,
  ...
  for (int j = 0; j < M; j++)
  {
    ...
    for (int k = 0; k < n; k++)
    {
      ...
      arma::colvec res = py->at(k) -
        ...
        mu(j);
      sigma(j) += pxi_n->at(k,j) * res(0) * res(0);
    }
...
// [[Rcpp::export]]
SEXP EMcppARMSIHPenalized (Rcpp::NumericVector y_rcpp,
                  ...
                  double transition_probs_max,
                  double sigma_min,
                  Rcpp::NumericVector sigma_null_rcpp,
                  double penalty_term)
{
  // conversion first
  ...
  arma::colvec sigma_null(sigma_null_rcpp.begin(), sigma_null_rcpp.size(), false);
  ...
  {
    index_exit++;
    Xi 		e_step = ExpectationStepARMSIHPenalized(&y, &y_lagged, &z_dependent, &z_independent,
                          &theta_updated, sigma_null, penalty_term);
    ...
    theta = theta_updated;
    theta_updated = MaximizationStepARMSIHPenalized(&y, &y_lagged,
