/*
// Computes estimates for AR-MSM models with EM-algorithm.
    ...
    for (int j_M = 0; j_M < M_reduced; j_M++)
		{
			for (int j_extra = 0; j_extra < M_extended_block; j_extra++)
			{
		      ...
		      pz_independent->row(k) * ptheta->gamma_independent - ptheta->mu(j_M);
				for (int lag = 0; lag < s; lag++)
				{
					...
					xi_k_t_jk -= ptheta->beta(lag) * // WATCH: beta non-switching
		    ...
		    xi_k_t(j,k) *= xi_k_t(j,k);
				xi_k_t(j,k) = xi_k_t(j,k) /
          (2 * (ptheta->sigma(0) * ptheta->sigma(0))); // WATCH: sigma non-switching
				...
				ratios[j] = xi_past_t(j, k) / ptheta->sigma(0); // WATCH: sigma non-switching
			}
		}
...
Xi ExpectationStepARMSM	(arma::colvec* py,
                          arma::mat* py_lagged,
                          arma::mat* pz_dependent,
                          arma::mat* pz_independent,
                          Theta* ptheta,
                          arma::imat* pstate_conversion_mat)
{
  ...
  Xi filter = FilterARMSM(py, py_lagged, pz_dependent, pz_independent,
...
Theta MaximizationStepARMSM (arma::colvec* py,
                            arma::mat* py_lagged,
                            arma::mat* pz_dependent,
                            ...
                            arma::imat* pstate_conversion_mat)
{
  ...
  arma::mat xi_n_sigmasq = *pxi_n /
    (ptheta0->sigma(0) * ptheta0->sigma(0)); // pi_{kx}/sigma_(x_1)^2 WATCH: H case?
  ...
  // 2-1. beta (non-switching)
  arma::mat 	beta_divisor(s, s, arma::fill::zeros); // WATCH: A case?
  arma::colvec 	beta_numerator(s, arma::fill::zeros); // WATCH: A case?
  ...
  for (int j = 0; j < M; j++)
  {
    ...
    for (int k = 0; k < n; k++)
    {
      ...
      beta_divisor += xi_n_sigmasq(k,j) *
        beta_jk_coeff * beta_jk_coeff.t();
      beta_numerator += xi_n_sigmasq(k,j) * beta_jk_coeff *
        (py->at(k) - ptheta0->mu(current_index)); // FIXIT: assume no regressors
    }
  }

  beta = arma::solve(beta_divisor, beta_numerator);
  ...
  for (int j = 0; j < M; j++)
  {
    ...
    for (int k = 0; k < n; k++)
    {
      ...
      for (int lag = 0; lag < s; lag++)
      {
        ...
        remainder_kj -= beta(lag) * beta_skj_coeff; // WATCH: A case?
      }
  ...
  for (int j = 0; j < M; j++)
  {
    ...
    for (int j_M = 0; j_M < M_reduced; j_M++)
    {
      ...
      mu_j_coeff -= arma::accu(ptheta0->beta); // WATCH: A case?
  ...
  for (int j = 0; j < M; j++)
    sigma(0) += sum(pxi_n->col(j) % (remainders.col(j) % remainders.col(j)));
  sigma(0) = std::max(sqrt(sigma(0) / sum_xi_n), sigma_min); // impose the hard constraint
...
// [[Rcpp::export]]
SEXP EMcppARMSM (Rcpp::NumericVector y_rcpp,
        					...
        					)
{
  ...
  {
    index_exit++;
    Xi 		e_step = ExpectationStepARMSM(&y, &y_lagged, &z_dependent, &z_independent,
    ...
    theta = theta_updated;
    theta_updated = MaximizationStepARMSM(&y, &y_lagged,
