                            ...
                            double sigma_min)
{
  ...
  if (arma::trace(transition_probs) < 1 + EPSILON_QUZHUO_BOUND)
  {
    ...
    // renormalize other non-persistant probabilities accordingly
    for (int i = 0; i < M; i++)
    {
      double revised_pivot = transition_probs(i,i);
      transition_probs.row(i) *= (1 - revised_pivot) / sum(transition_probs.row(i));
      transition_probs(i,i) = revised_pivot; // back to revised value for pivot
    }
  }
