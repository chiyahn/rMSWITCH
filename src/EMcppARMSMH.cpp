/*
 // Computes estimates for AR-MSMH models with EM-algorithm.
...
Xi FilterARMSMH (arma::colvec* py,
                 arma::mat* py_lagged,
    ...
    for (int j_M = 0; j_M < M_reduced; j_M++)
    {
      for (int j_extra = 0; j_extra < M_extended_block; j_extra++)
      {
        ...
        for (int lag = 0; lag < s; lag++)
        {
          ...
          xi_k_t_jk -= ptheta->beta(lag) *  // WATCH: beta switching
...
Xi ExpectationStepARMSMH	(arma::colvec* py,
                          arma::mat* py_lagged,
                          arma::mat* pz_dependent,
                          arma::mat* pz_independent,
                          Theta* ptheta,
                          arma::imat* pstate_conversion_mat)
{
  ...
  Xi filter = FilterARMSMH(py, py_lagged, pz_dependent, pz_independent,
...
Theta MaximizationStepARMSMH (arma::colvec* py,
                              arma::mat* py_lagged,
                              arma::mat* pz_dependent,
...
// [[Rcpp::export]]
SEXP EMcppARMSMH (Rcpp::NumericVector y_rcpp,
...
)
{
  ...
  {
    index_exit++;
    Xi 		e_step = ExpectationStepARMSMH(&y, &y_lagged, &z_dependent, &z_independent,
    ...
    theta = theta_updated;
    theta_updated = MaximizationStepARMSMH(&y, &y_lagged,
