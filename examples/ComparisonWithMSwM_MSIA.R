...
model=lm(y ~ 1)
msmFit(model, k=M, p=s, sw=c(T,T,T,F)) # MSwM (dependent mu, dependent beta1 beta2, independent sigma)
...
msar.model$log.likelihood
model=lm(y ~ 1)
msmFit(model, k=M, p=s, sw=c(T,T,F)) # MSwM (dependent mu, dependent beta1, independent sigma)
...
msmFit(model, k=M, p=s, sw=c(T,T,F,F,T,F)) # MSwM (dependent mu, z1, independent z2, z3, dependent beta1, independent sigma)
